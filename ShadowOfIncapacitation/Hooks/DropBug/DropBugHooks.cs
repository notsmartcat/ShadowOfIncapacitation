...
internal class Hooks
{
    ...
    #region DropBug
    static void DropBugCollide(On.DropBug.orig_Collide orig, DropBug self, PhysicalObject otherObject, int myChunk, int otherChunk)
    {
        orig(self, otherObject, myChunk, otherChunk);

        if (!IsIncon(self) || otherObject is not Creature)
        {
            return;
        }
    ...
    static void DropBugUpdate(On.DropBug.orig_Update orig, DropBug self, bool eu)
    {
        ...
        void AIUpdate()
        {
            ...
            ai.noiseTracker.hearingSkill = ai.bug.sitting ? 1f : 0.3f;
            ...
            else if (ai.behavior == DropBugAI.Behavior.EscapeRain)
            {
