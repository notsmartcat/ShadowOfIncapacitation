...
internal class Hooks
{
    ...
    static void BigSpiderCollide(On.BigSpider.orig_Collide orig, BigSpider self, PhysicalObject otherObject, int myChunk, int otherChunk)
    {
        ...
        if (!inconstorage.TryGetValue(self.abstractCreature, out InconData data) || !IsIncon(self) || otherObject is not Creature)
        {
            return;
        }
    ...
    static void BigSpiderUpdate(On.BigSpider.orig_Update orig, BigSpider self, bool eu)
    {
        ...
        void Swim()
        {
            ...
            if (movementConnection == default && Math.Abs(self.abstractCreature.pos.y - self.room.defaultWaterLevel) < 4)
            {
    ...
    static void BigSpiderInconAct(InconData data)
    {
        if (data.stunCountdown > 0)
        {
            return;
