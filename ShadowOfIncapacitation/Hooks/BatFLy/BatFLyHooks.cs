...
internal class Hooks
{
    public static void Apply()
    {
        ...
        On.Fly.WaterBehavior += FlyWaterBehavior;
    }
    ...
    static void FlyWaterBehavior(On.Fly.orig_WaterBehavior orig, Fly self)
    {
        orig(self);

        if (!inconstorage.TryGetValue(self.abstractCreature, out InconData data) || !IsInconBase(self) || self.graphicsModule == null)
        {
            return;
        }
    ...
    static void FlyAIUpdate(On.FlyAI.orig_Update orig, FlyAI self)
    {
        ...
        if (data.stunTimer > 0 && (data.rescueCandidate == null || (data.rescueCandidate as Fly).grasps[0] == null || (data.rescueCandidate as Fly).grasps[0].grabbedChunk.owner != self.fly))
        {
            data.stunTimer -= 1;
        }
        if (data.stunCountdown > 0 && (data.rescueCandidate == null || (data.rescueCandidate as Fly).grasps[0] == null || (data.rescueCandidate as Fly).grasps[0].grabbedChunk.owner != self.fly))
        {
